...
Test, NYC Taxi veri setinin Ocak 2024 Parquet dosyası üzerinde gerçekleştirilmiştir. Görev yönergesinde belirtilen "bir ay, ~3 GB" veri hacmi, modern Parquet sıkıştırması sayesinde diskte ~50 MB yer kaplamaktadır. Ancak bellek (RAM) testleri, verinin işlenmesi sırasındaki gerçek kaynak tüketimini ortaya koymuştur.
...
DuckDB kullanılarak, sunucu kurulumuna gerek kalmadan doğrudan dosya üzerinden 10 farklı iş sorusu (gruplama, tarih filtreleme, koşullu sayım vb.) tek bir işlem bloğunda başarıyla ve saniyeler altında çalıştırılmıştır.
